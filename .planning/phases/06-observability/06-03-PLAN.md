...
<task type="auto">
  <name>Task 2: Add agent-level spans to subclass methods (optional enhancement)</name>
  <files>hfs/agno/teams/base.py</files>
  <action>
Add optional agent-level span creation for finer granularity. This is within AgnoTriad and wraps individual agent calls if the team exposes them.

Since Agno's team.arun() handles agent coordination internally, we can add agent-level attributes to the triad span OR create a helper that subclasses can use.
...
3. Optionally, add model information from model_selector if available:
   ```python
...
`pytest hfs/tests/test_observability_agents.py -v` passes all tests.
  ...
  <done>Agent observability tests pass, verifying triad span creation, attributes, token recording, and error handling.</done>
</task>
...
- Tests verify span creation, attributes, token recording, and error handling
</success_criteria>
