...
<task type="auto">
  <name>Task 3: Create metrics.py and config.py</name>
  <files>hfs/observability/metrics.py, hfs/observability/config.py</files>
  <action>
...
Add shutdown_telemetry() function to __init__.py that calls force_flush and shutdown on both providers.
...
```
  ...
  <done>MeterProvider initializes with LLM-appropriate histogram buckets (0.1s to 60s), config utilities available, shutdown handler registered.</done>
</task>
