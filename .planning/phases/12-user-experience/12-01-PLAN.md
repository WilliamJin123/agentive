...
must_haves:
  truths:
    - "Config loads from ~/.hfs/config.yaml if it exists"
    - "Config loads from .hfs/config.yaml and overrides global"
    - "Environment variables override config file values"
    - "/config shows current effective configuration"
    - "/config set key value updates and persists preference"
    - "Invalid config values are rejected with explanation"
  artifacts:
...
- `get_effective_config() -> dict`: Return merged config as dict for display
